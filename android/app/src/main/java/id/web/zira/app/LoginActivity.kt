...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Cek Deep Link Callback dari Google OAuth (zira://auth?token=...&name=...)
        intent?.data?.let { uri ->
    ...
    private fun setupListeners() {
        ...
        binding.btnGoogleLogin.setOnClickListener {
            // Membuka Google Login via Chrome Custom Tabs dengan akun Google HP
    ...
    private fun handleOAuthSuccess(token: String, name: String) {
        setLoading(true)
        // Ambil data profile lengkap dari endpoint /api/v1/auth/me
        ApiClient.get("/api/v1/auth/me", token, LoginResponse::class.java) { success, resp, err ->
            runOnUiThread {
                setLoading(false)
